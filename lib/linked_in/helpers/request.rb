    ...
    module Request
      ...
      private
        ...
        def raise_errors(response)
          ...
          status = response.response.status.to_i
          ...
          when 404
            raise LinkedIn::Errors::NotFoundError, "(#{status}): #{data.message}"
          when 500
            raise LinkedIn::Errors::InformLinkedInError, "LinkedIn had an internal error. Please let them know in the forum. (#{status}): #{data.message}"
          when 502..503
            raise LinkedIn::Errors::UnavailableError, "(#{status}): #{data.message}"
          end
        end
